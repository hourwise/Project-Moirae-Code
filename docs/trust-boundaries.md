...
| External CLIs and child processes            | Supervisor spawn path is stubbed; [sandbox-adapter](../packages/sandbox-adapter/src/adapter.ts) defines mode selection, validation, and approval previews for governed execution | Sandbox config validation and risk-to-mode mapping are implemented; process spawning is stubbed | Any CLI launched outside future tool runners bypasses Moirae                                                | Sandbox, cap output, restrict working directory, and limit child processes                          | Implemented at contract/validation level; enforcement not yet live |
...
| Local credentials                            | Secret-broker interface, in-memory store, bounded `SecretLeaseManager`, and one-shot scoped delivery are implemented | Host broker retains raw material; lease metadata is safe for audit and delivery consumes before invoking the effect boundary | Production OS-keychain implementation and complete provider interception remain outside this repo          | Use OS keychain-backed brokering and avoid repo, chat, and audit storage                            | Lease boundary implemented; production custody not yet claimed |
| Firecracker guest channel                    | Fixed-profile VMM supervisor, allowlisted constrained vsock channel, guest workload start/cancel, and scoped credential delivery are implemented | Host controls the fixed session/CID/port envelope and rejects arbitrary methods, drift, replay, and oversized frames | Linux/KVM device proof, guest agent, and OS-enforced containment remain environment-dependent                 | Keep the channel fixed-purpose and fail closed on identity, size, timeout, or transport failure   | Portable boundary implemented; Linux containment not claimed |
...
- Sandbox execution mode selection, config validation, approval previews, and path/network boundary checks are implemented and tested as library behavior.
